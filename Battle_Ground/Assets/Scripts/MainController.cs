...
public class MainController : MonoBehaviour {
    ...
    void Update()
    {
        ...
        //Vector3 move = new Vector3(Mathf.Clamp(-pos.x, -35, 35) * dragSpeed, 0, Mathf.Clamp(-pos.y, -35, 35) * dragSpeed);
        //mainCameraHolder.transform.Translate(move, Space.World);
        ...
        if (Input.GetMouseButtonDown(0) && !isPlaced)
        {
            ...
            if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out hitInfo))
            {
                if (hitInfo.transform.gameObject.tag == "Respawn")
                {
                    ...
                    if (building.GetComponent<ConstructionBase>().IsConstructed())
                    {
                        building.AddComponent<CollisionCheck>();
                        Vector3 Inpos = Camera.main.ScreenToWorldPoint(new Vector3(Input.mousePosition.x, Input.mousePosition.y, -Camera.main.transform.position.z));
                        building.transform.localPosition = new Vector3(Inpos.x, 0, Inpos.y);
                    }                    
                }
            }
        }
...
*/
            ...
            if (Input.GetMouseButtonDown(0))
            {
                ...
                else
                {
                    isPlaced = true;
                    isReplaced = false;
                    building.GetComponent<ConstructionBase>().SetIsBuildingPlaced();
